...
public class GlobalExceptionHandler {
    ...
    public <T> Result<T> handleBizException(BizException e) {
        log.error("业务异常，异常原因：{}", e.getCustomCode() != null ? e.getCustomCode() : e.getMessage(), e);
        if (e.getResultCode() != null) {
            return Result.failed(e.getResultCode());
        } else if (e.getCustomCode() != null) {
            return Result.failedByCode(e.getCustomCode());
        }
        return Result.failed(e.getMessage());
    }
    ...
    public <T> Result<T> handleHystrixRuntimeException(HystrixRuntimeException e) {
        log.error(e.getMessage(), e);
        if (e.getCause() instanceof BizException) {
            BizException be = (BizException) e.getCause();
            if (be.getResultCode() != null) {
                return Result.failed(be.getResultCode(), be.getMessage());
            } else if (be.getCustomCode() != null) {
                return Result.failedByCode(be.getCustomCode());
            }
        }
    ...
    public <T> Result<T> handleDuplicateKeyException(DuplicateKeyException e) {
        log.error(e.getMessage(), e);
        if (e.getCause().getMessage().contains("mobile")) {
            return Result.failedByCode("error.mobileDuplicate");
        } else if (e.getCause().getMessage().contains("email")) {
            return Result.failedByCode("error.emailDuplicate");
        } else if (e.getCause().getMessage().contains("company_bank_card_group")) {
            return Result.failed(ResultCode.DATABASE_COLUMN_DUPLICATE);
        } else if (e.getCause().getMessage().contains("company_bank_card_bank_card_no_uindex")) {
            return Result.failed(ResultCode.BANK_CARD_DUPLICATE);
        } else if (e.getCause().getMessage().contains("company_bank_card.code_UNIQUE")) {
            return Result.failed(ResultCode.BANK_CARD_CODE_DUPLICATE);
        }
        return Result.failed(ResultCode.DATABASE_COLUMN_DUPLICATE);
    }
