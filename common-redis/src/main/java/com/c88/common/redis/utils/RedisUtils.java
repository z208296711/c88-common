package com.c88.common.redis.utils;
...
public class RedisUtils {
     ...
     * @return
     */
    public Map<String, Long> getMultiExpires(String... keys) {
        ...
        for (int i = 0; i < keys.length; i++) {
            if (result.get(i) == null)
                map.put(keys[i], null);
            else if (((Long) result.get(i)).intValue() > 0)
                map.put(keys[i], (Long) result.get(i));
        }
        return map;
    }
    ...
    public static String buildKey(Object... keys) {
        return Arrays.stream(keys).map(Objects::toString).collect(Collectors.joining(":"));
    }

    public  <T> T  executeLua(String redisScript,
                            Class<T> clazz,
                            List<String> keys,
                            Object... args) {
        return redisTemplate.execute(new DefaultRedisScript<>(redisScript, clazz),
                keys,
                args);
    }
